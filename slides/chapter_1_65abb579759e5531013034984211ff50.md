...
`@script`
Here we have our artists table from before. Our database will also contain a songs table containing data aout all the songs for our music app. In this table you can see we have an artists_id column. This is a foreign key column which is referencing the primary key id column in the artists table. So where we see a 1 in the artists_id column in the songs table, this is referring to Lady Gaga. Where we see a 2, Daft Punk and so on.
